// lib/telemetry_widgets_human.dart
import 'package:flutter/material.dart';

class TelemetryHuman extends StatelessWidget {
  final Map<String, dynamic> stats;
  final String partenza;
  final String arrivo;
  final String tappa;
  final bool isAndataRitorno;
  final TimeOfDay departureTime; // <-- Ricevuto dal centralino

  const TelemetryHuman({
    Key? key, 
    required this.stats, 
  ...
  // Calcola l'ora esatta di arrivo basata sulla velocità del pedone
  ...
  Widget build(BuildContext context) {
    ...
    double kcal = (stats['litri_totali_normale'] ?? 0.0).toDouble(); 

    // --- 1. HEADER STILIZZATO (Itinerario Professionale) ---
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: Colors.deepOrangeAccent.withOpacity(0.4), width: 2),
            boxShadow: [
              BoxShadow(color: Colors.deepOrangeAccent.withOpacity(0.1), blurRadius: 20, spreadRadius: 2)
            ],
          ),
          child: Row(
  ...
  // Helper per le colonne delle statistiche
